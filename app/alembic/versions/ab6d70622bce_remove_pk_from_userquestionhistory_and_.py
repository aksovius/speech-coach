...
def upgrade():
    op.drop_constraint('user_question_history_pkey', 'user_question_history', type_='primary')
    op.add_column('user_question_history', sa.Column('id', sa.Integer(), primary_key=True, autoincrement=True))
    op.create_primary_key('user_question_history_pkey', 'user_question_history', ['id'])

def downgrade():
    op.drop_constraint('user_question_history_pkey', 'user_question_history', type_='primary')
    op.drop_column('user_question_history', 'id')
    op.create_primary_key('user_question_history_pkey', 'user_question_history', ['user_id', 'question_id'])
