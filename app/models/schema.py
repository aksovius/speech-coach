from sqlalchemy import (
    Column, Integer, BigInteger, Text, Boolean, ForeignKey, TIMESTAMP, JSON, Index, String, CheckConstraint
)
...
class Question(Base):
    ...
    __table_args__ = (
        Index('idx_questions_category_active', 'category', 'is_active'), 
    )
...
class Media(Base):
    ...
    __table_args__ = (
        Index("idx_media_source", "source_type", "source_id"),
    )
...
class QuestionTag(Base):
    ...
    __table_args__ = (
        Index('idx_question_tags_tag', 'tag'), 
    )
...
class UserAnswer(Base):
    ...
    score_overall = Column(Integer, CheckConstraint('score_overall >= 0 AND score_overall <= 100')) 
    created_at = Column(TIMESTAMP, server_default=func.now())
    media = relationship(
        "Media",
        primaryjoin=lambda: and_(
